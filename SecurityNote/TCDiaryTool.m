...
+(NSMutableArray *)queryWithKeywords:(NSString *)words{
    ...
    [_queue inDatabase:^(FMDatabase *db)
     {
         ...
         FMResultSet *rs = nil;
         NSString *NSsql=[NSString stringWithFormat:@"select * from securitynote where title like '%%%@%%' order by ids asc;",words];
         rs = [db executeQuery:NSsql];
