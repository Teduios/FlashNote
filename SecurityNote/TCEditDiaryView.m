...
@interface TCEditDiaryView ()<TCDatePickerViewDelegate,UITextViewDelegate,UIScrollViewDelegate,AVSpeechSynthesizerDelegate>
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        //创建/添加item
        UILabel *readItem = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 30, 40)];
        readItem.font = [UIFont systemFontOfSize:18];
        readItem.text = @"朗读";
        readItem.textColor = [UIColor whiteColor];
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(clickReadItem)];
        [readItem addGestureRecognizer:tap];
        readItem.userInteractionEnabled = YES;
        [self.navigationItem setRightBarButtonItems:@[rightItem]];
        self.readLabel = readItem;
        self.navigationItem.titleView = readItem;
...
- (void)clickReadItem {
    //判断是否正在朗读, 如果在读,停止
    if (self.speechSynt.speaking) {
        [self.speechSynt stopSpeakingAtBoundary:AVSpeechBoundaryWord];
        return;
    }
    //实现朗读的功能(语言+朗读内容)
    //创建说话方式对象
    NSString *str =[self.titleField.text stringByAppendingPathComponent:self.detailView.text];
    ...
    //打印当前可以支持所有语言的名字
    NSArray *voiceArray = [AVSpeechSynthesisVoice speechVoices];
    NSLog(@"支持的所有语言:%@", voiceArray);
}
#pragma mark - AVSpeech Delegate
//开始读时间点
- (void)speechSynthesizer:(AVSpeechSynthesizer *)synthesizer didStartSpeechUtterance:(AVSpeechUtterance *)utterance {
//    self.navigationItem.rightBarButtonItem.title = @"停止";
    self.readLabel.text = @"停止";
}
//读完时间点
- (void)speechSynthesizer:(AVSpeechSynthesizer *)synthesizer didFinishSpeechUtterance:(AVSpeechUtterance *)utterance {
//    self.navigationItem.rightBarButtonItem.title = @"朗读";
     self.readLabel.text = @"朗读";
}
//取消时间点
